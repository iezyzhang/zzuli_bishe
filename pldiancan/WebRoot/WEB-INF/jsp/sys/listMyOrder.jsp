...
<div class="pageHeader">
	...
	<div class="searchBar">
		<ul class="searchContent">
			...
			<li>
				<label>订单日期：</label>
				<input type="text" name="s_userOrder.addDate" value="" readonly="readonly" class="date"/>
			</li>
			<li>
				<label>状态：</label>
...
<!-- 			<li><a class="add" href="${ctx}/sys/add2BookType.action" target="navTab"  rel="baseAdd" title="Add One"> <span>Add</span></a></li> -->
			<li><a class="edit" href="${ctx}/sys/getMyOrder.action?uid={sid_select}" target="navTab" rel="baseAdd" warn="选择需要操作的行" title="订单明细"><span>查看订单</span></a></li>
<!-- 			<li><a title="Delete All Selected?" target="selectedTodo" rel="ids" postType="string" href="${ctx}/sys/deleteBookType.action" class="delete" warn="选择需要操作的行"><span>Delete</span></a></li> -->
		</ul>
	</div>
...
<!-- 				<td><input name="ids" value="${item.id }" type="checkbox"></td> -->
				...
				<td>${item.price}</td>
				<td>${item.userOrder.statusStr}</td>
				<td>${item.status}</td>
				<td>${item.book.sid}</td>
				<td>${item.book.name}</td>
				<td width="70">
					<a title="订单信息"  href="${ctx}/sys/getMyOrder.action?uid=${item.id}" class="btnEdit" target="navTab" rel="baseAdd">Edit One</a>
